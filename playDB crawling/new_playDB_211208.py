...
def get_naver_url(empty_frame):
    ...
    for i, row in tqdm(empty_frame.iterrows(), total=empty_frame.shape[0]):
        ...
        try :
            if naver_soup.select("div[class='cm_top_wrap _sticky _custom_select _header']"):
                ntitle_list = []
                try:
                ...
                if '같은 이름 다른 인물' in naver_soup.text:
                    try:
                        ...
                        doScrollDown(10, naver_drv)

                        naver_source1 = naver_drv.page_source
                        naver_soup1 = BeautifulSoup(naver_source1, "html.parser")

                        #                     people = naver_soup.find_all('ul', {'class': '_list'})
                        people = naver_soup1.find_all('div', {'class': 'item'})
                        # print('메인 - 동명이인 - 전체보기')
                    ...
                    for p in range(len(people)):
                        ...
                        if people[p].get('href'):
                            people_url_ = naver_base + people[p].get('href')

                        elif people[p].select('a'):
            ...
            # 2. 세로 리스트
                ...
                if '같은 이름 다른 인물' in naver_soup.text:
                    try:
                        ...
                        naver_source2 = naver_drv.page_source
                        naver_soup2 = BeautifulSoup(naver_source2, "html.parser")

                        #                     people = naver_soup.find_all('ul', {'class': '_list'})
                        people = naver_soup2.find_all('div', {'class': 'item'})
                        # print('메인 - 동명이인 - 전체보기')
            ...
            # 4. 가로 리스트 / 프로필 없음
                ...
                if '같은 이름 다른 인물' in naver_soup.text:
                    try:
                        ...
                        naver_source3 = naver_drv.page_source
                        naver_soup3 = BeautifulSoup(naver_source3, "html.parser")

                        #                     people = naver_soup.find_all('ul', {'class': '_list'})
                        people = naver_soup3.find_all('div', {'class': 'item'})
                        # print('가로 - 동명이인 - 전체보기')
