...
def getMaimoviePeople() :
    ...
    naver_crew = naver_crew.drop_duplicates(subset='n_people_code', keep='first')  # 중복 제거
    naver_crew = naver_crew.dropna(subset=['n_people_code'])  # null 제거
...
gaon_cnt = count('youtube') + count('smr') + count('vlive') + count('익디') + count('music_streaming') + count('youtube_music')
all_cnt = gaon_cnt + count('공연') + count('영화') + count('방송') + count('광고') + count('인플루언서') + count('버추얼스타')

print('*** 데일리 인물 등록 및 매핑 현황입니다. @Sanguine Kim @오승훈')
print('- 가온(youtube, smr, vlive, music_streaming, youtube_video) & 익디')
...
print('누적 등록 개수 :', tv.date.count(), '/ 9308')
